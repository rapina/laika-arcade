...
// canvas에 keydown을 바인딩한 게임은 canvas가 포커스를 가져야만 키 입력을
// 받는다. 브라우저에 따라 클릭만으로는 포커스가 이동하지 않으므로(macOS
// Safari, Firefox) 마운트 직후와 게임 영역 포인터 입력 때 직접 포커스를 준다.
function focusGameSurface(event) {
  const target = event?.target;
  if (target instanceof Element && target.closest("button, a, input, select, textarea, [contenteditable]")) return;
  const surface = root.querySelector("[tabindex]") ?? root.querySelector("canvas");
  if (surface instanceof HTMLElement) surface.focus({ preventScroll: true });
}
root.addEventListener("pointerdown", focusGameSurface);
